...
# Load models results
data("train_full_xgb_rslt")
data("train_q20_xgb_rslt")
data("train_q10_xgb_rslt")
data("train_q05_xgb_rslt")

# Load workflow
data("train_full_xgb_wfl")
data("train_q20_xgb_wfl")
data("train_q10_xgb_wfl")
data("train_q05_xgb_wfl")
...
for(d in dat_vect){
  ...
  rslt <- get(paste(d, "xgb", "rslt", sep = "_"))
  ...
  # fit bst
  fit_bst <- get(paste(d, "xgb", "wfl", sep = "_")) %>% 
    tune::finalize_workflow(bst) %>% 
    parsnip::fit(dat)
  # fit onese
  fit_onese <- get(paste(d, "xgb", "wfl", sep = "_")) %>% 
    tune::finalize_workflow(onese) %>% 
    parsnip::fit(dat)
  
  fit_bst_name <- paste(d, "xgb_fit_bst", sep = "_") 
  assign(fit_bst_name, fit_bst)
  
  fit_onese_name <- paste(d, "xgb_fit_onese", sep = "_") 
